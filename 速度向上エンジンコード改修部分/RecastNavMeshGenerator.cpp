...
void FRecastNavMeshGenerator::Init()
{
	...
	// [NavMeshSerializer Mod] 元のコードではNumberOfWorkerThreads*2でクランプされていたが、
	// タスクはI/O待ちやジオメトリ収集待ちが発生するため、ワーカースレッド数以上の
	// 同時タスクを許可することでスループットが向上する。
	// MaxSimultaneousTileGenerationJobsCountが設定されていればその値を優先する。
	const int32 OwnerMaxJobs = GetOwner() ? GetOwner()->GetMaxSimultaneousTileGenerationJobsCount() : INT_MAX;
	if (OwnerMaxJobs > NumberOfWorkerThreads * 2)
	{
		// ユーザーが明示的に大きな値を設定した場合はそれを尊重
		MaxTileGeneratorTasks = OwnerMaxJobs;
	}
	else
	{
		// デフォルト動作（従来通り）
		MaxTileGeneratorTasks = FMath::Min(FMath::Max(NumberOfWorkerThreads * 2, 1), OwnerMaxJobs);
	}
	UE_LOG(LogNavigation, Log, TEXT("Using max of %d workers to build navigation. (WorkerThreads=%d, OwnerMax=%d)"), MaxTileGeneratorTasks, NumberOfWorkerThreads, OwnerMaxJobs);
	NumActiveTiles = 0;
...
void FRecastNavMeshGenerator::TickAsyncBuild(float DeltaSeconds)
{
	...
	// [NavMeshSerializer Mod] 元のコードではNavSys->GetNumRunningBuildTasks()で
	// NavigationSystem全体の実行中タスク数を取得していたが、これだと複数エージェントの
	// NavMeshが同時ビルドする際にジョブ枠を分け合ってしまう。
	// 各Generatorが自分のRunningDirtyTiles数で制限するように変更。
	const int32 NumRunningTasks = RunningDirtyTiles.Num();
